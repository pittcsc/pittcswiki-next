...
function calculateChangePercentage(originalContent, newContent) {
  ...
  let matchingLines = 0;
  const originalLineSet = new Set(originalLines.map(line => line.trim().toLowerCase()));
  const updatedLineSet = new Set(updatedLines.map(line => line.trim().toLowerCase()));
...
async function getGitMetadata(filePath) {
  try {
    ...
    for (let i = 1; i < reversedCommits.length; i++) {
      ...
      const changePercentage = calculateChangePercentage(lastKnownContent, commitContent);
    ...
    try {
      ...
      const finalChangePercentage = calculateChangePercentage(lastKnownContent, currentContent);
    ...
    for (const filePath of guideFiles) {
      ...
      try {
        ...
        metadata[guideId] = {
          ...
          originalAuthorEmail: gitData.originalAuthorEmail || gitData.authorEmail,
        ...
        console.log(`✓ ${relativePath} (${wordCount} words, ${readingTime} min read)`);
      } catch (error) {
